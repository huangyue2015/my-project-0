package com.hbase.canal.pool.db;
...
public class ConnectionPool implements IConnectionPool
{
	...
	public void init()
	{
		...
		{
			//获得初始化的连接数
			int initpool = dbbean.getInitpool() == 0 ? 5 : dbbean.getInitpool();
	...
	public Connection getConnection()
	{
		...
		{
			//判断是否超过最大连接限制
			if(coutActive < dbbean.getMaxpool())
			{
	...
	public synchronized void releaseConnection(Connection conn)
	{
		try
		{
			if(isValid(conn) && dbbean.getMaxpool()<freeConnection.size())
			{
				freeConnection.add(conn);
	...
	private synchronized Connection newConn() throws ClassNotFoundException, SQLException
	{
		...
		{ 
			...
			Class.forName(path);
			String url = String.format("jdbc:postgresql://%s:%s/%s", dbbean.getUri(),dbbean.getPort(),dbbean.getDb());
			conn = DriverManager.getConnection(url, dbbean.getUid(), dbbean.getPwd());
		}
		return conn;
	}
